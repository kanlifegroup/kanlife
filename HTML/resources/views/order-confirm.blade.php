         ...
         <div class="row">
           <div class="col-md-12 mt-1 mb-1 pt-1 pb-1" align="center">
                ...
                <div class="col fs20">
                  ...
                  @endif
                  @if(!empty($vat_price))
                  <label class="mt-2 mb-2">Tax Price (VAT) <span>:</span> {{ $allsettings->site_currency_symbol }}{{ $vat_price }}</label><br/>
                  @endif
                ...
                @if($payment_method=="coinpayments")
                 <form method="POST" action="{{ route('confirm-coinpayments') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                 {{ csrf_field() }}
                        <input type="hidden" name="email" value="{{ Auth::user()->email }}"> 
                        <input type="hidden" name="purchase_token" value="{{ $purchase_token }}">
                        <input type="hidden" name="amount" value="{{ base64_encode($total_price) }}"> 
