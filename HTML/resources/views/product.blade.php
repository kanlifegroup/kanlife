      ...
      <div class="col-md-12 mt-3">
        ...
        {{ csrf_field() }}
          <h3>{{ $shop->product_name }}</h3>
            ...
            @if($shop->product_sku != "")<div class="mt-2"><strong>{{ Helper::translation(1929,$translate,'') }} :</strong> <span>{{ $shop->product_sku }}</span></div>@endif
            @if($shop->product_type != 'digital')
            <div class="mt-2"><strong>{{ Helper::translation(2062,$translate,'') }} :</strong> @if($shop->product_stock != 0)<span class="theme-color">{{ Helper::translation(2063,$translate,'') }} ({{ $shop->product_stock }})</span>@else<span class="red-color">{{ Helper::translation(2064,$translate,'') }} ({{ $shop->product_stock }})</span>@endif</div>
            @endif
            @php if($shop->product_condition == 'new'){ $badg = "badge badge-warning"; } else { $badg = "badge badge-secondary"; } @endphp
            @if($shop->product_condition != "")<div class="mt-2"><strong>{{ Helper::translation(1950,$translate,'') }} :</strong> <span class="{{ $badg }}">{{ $shop->product_condition }}</span></div>@endif
            @if($shop->product_brand != "")<div class="mt-2"><strong>{{ Helper::translation(1947,$translate,'') }} :</strong> <span class="badge badge-info">{{ $shop->brand_name }}</span></div>@endif
