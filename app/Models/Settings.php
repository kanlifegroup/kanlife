...
class Settings extends Model
{
  ...
  /* postcode */
   public static function getpostcodeData()
	  {
	
		$value=DB::table('postcode')->orderBy('postcode_value', 'asc')->get(); 
		return $value;
		
	  }
  
  public static function savepostcodeData($data){
   
      DB::table('postcode')->insert($data);
     
 
    }
	
	public static function deletePostcodedata($cid){
    
	DB::table('postcode')->where('postcode_id', '=', $cid)->delete();	
	
	
  }
  
  public static function editPostcode($cid){
    $value = DB::table('postcode')
      ->where('postcode_id', $cid)
      ->first();
	return $value;
  }	
  
  public static function updatepostcodeData($cid,$data){
    DB::table('postcode')
      ->where('postcode_id', $cid)
      ->update($data);
  }
  ...
  public static function allCountry(){
	...
	{
    $get = DB::table('postcode')
      ->where('postcode_value', $postcode)
      ->get();
	$value = $get->count(); 
    return $value;
    }
