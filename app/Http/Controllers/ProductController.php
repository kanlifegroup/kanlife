...
class ProductController extends Controller
{
  ...
  public function price_enquiry($id)
    ...
    $final += $gst;
	  ...
	  $data = array('is_categories'=>true,'setting' => $setting, 'cart' => $cart, 'subtotal' => $subtotal,'shipping'=>$shipping,'gst'=>$gst, 'coupon_code' => $coupon_code, 'new_price' => $new_price, 'coupon_discount' => $coupon_discount, 'final' => $final, 'get_payment' => $get_payment, 'order_numbers' => $order_numbers, 'product_numbers' => $product_numbers, 'product_names' => $product_names, 'checkout_details' => $checkout_details, 'checkout_data' => $checkout_data);
    ...
    $instamojo_mode = 'api';// 'api' & 'test'
	   ...
	   if($additional->tax_display == 1)
	   {
			...
			{
			    //$loop_count = count($cart_data['display']);
	...
	public function redirectToGateway()
  {
	...
	public function confirm_coinpayments(Request $request)
	{
	   $sid = 1;
	   $setting['setting'] = Settings::editGeneral($sid);
	   $extra['setting'] = Settings::editAdditional();
	   ...
	   /* coinpayments */
	   $coinpayments_merchant_id = $extra['setting']->coinpayments_merchant_id;
	   /* coinpayments */
	   $cancel_url = $website_url.'/cancel';
	...
	public function confirm_razorpay(Request $request)
	{
	   ...
	   $setting['setting'] = Settings::editGeneral($sid);
	   $extra['setting'] = Settings::editAdditional();
	   if($setting['setting']->site_currency_code != 'INR')
	   {
	   $convert = Currency::convert()->from($setting['setting']->site_currency_code)->to('INR')->amount(base64_decode($request->input('amount')))->round(2)->get();
	   $price_amt = $convert * 100;
	   }
	   else
	   {
	   $price_amt = base64_decode($request->input('amount')) * 100;
	   }
	    $item_names_data = $request->input('product_names');
		$purchase_token = $request->input('purchase_token');
	...
	public function view_coupon(Request $request)
	{
     ...
     $session_id = Session::getId();
	   ...
	   if($check_coupon == 1)
	   {
	      ...
	      $coupondata['get'] = Product::getCoupon($coupon,$user_id,$session_id);
