          ...
          <div class="product-count mt-3 mb-3" aos="fade-left">
            <form action="#" class="d-flex align-items-center">
              @if(isset($cart['qty']) && $cart['qty'] > 0)
                ...
                <a href="{{ url('/cart') }}" class="enquirys-btn mx-3 px-3 px-lg-5 text-light" style="border-radius: 5px;font-size: 1.4rem;">Go to cart</a>
              @else
                @if($shop->product_price != 0)
                  <a href="{{url('/add_to_cart').'/'.$shop->product_slug}}" class="enquirys-btn text-light" style="border-radius: 5px;font-size: 1.4rem;">Add To Cart</a>
                @else
                  @guest
                  <a href="javascript:void(0);" onclick="openModel('myModal')" class="enquirys-btn text-light" style="border-radius: 5px;font-size: 1.4rem;">Price Enquiry</a>
                  @else
                  <a href="{{url('/price_enquiry')}}/{{ base64_encode($shop->product_id) }}" class="enquirys-btn text-light" style="border-radius: 5px;font-size: 1.4rem;">Price Enquiry</a>
                  @endif
                @endif
              @endif
            </form>
